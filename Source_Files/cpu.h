...
class CPU {
 ...
 public:
  ...
  std::unordered_map<int, std::function<void(CPU&, int, int, int, int)>>
      ROptable;
  ...
  void subtract(const int reg_a, const int reg_b, const int reg_c,
                const int shift_value);
  void or_Op(const int reg_a, const int reg_b, const int reg_c,
             const int shift_value);
  void nor(const int reg_a, const int reg_b, const int reg_c,
           const int shift_value);

  void add(const int reg_a, const int reg_b, const int reg_c,
           const int shift_value);
  void shiftRightArithmetic(const int reg_a, const int reg_b, const int reg_c,
                            const int shift_value);
  void bitwise_and(const int reg_a, const int reg_b, const int reg_c,
                   const int shift_value);
  void jumpRegister(const int reg_a, const int reg_b, const int reg_c,
                    const int shift_value);

  void shiftLeftLogical(const int reg_a, const int reg_b, const int reg_c,
                        const int shift_value);
  void shiftRightLogical(const int reg_a, const int reg_b, const int reg_c,
                         const int shift_value);
  void setLessThan(const int reg_a, const int reg_b, const int reg_c,
                   const int shift_value);
  ...
  enum Opcode {
    ...
    jumpAndLinkCode = 61,
    R_TYPE = 62,  // note that all R-type instructions have same opcode, but
                  // diff function #s
  };
