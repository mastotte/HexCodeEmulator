...
void BananaOS::loop() {
  ...
  while (true) {
    ...
    while (bananaCPU.programCounter != 0) {
      ...
      // bananaMEM.write8(CONTROLLER_INPUT_MEMORY,
      // CharacterMaskMap[controllerInput]);
    }
  }
}
  ...
  if (bananaCPU.IOptable.find(opcode) != bananaCPU.IOptable.end()) {
    ...
    if (opcode == bananaCPU.R_TYPE) {
      ...
      if (bananaCPU.ROptable.find(function) != bananaCPU.ROptable.end()) {
        ...
        reg_c = reg_c >> 27;
        // std::cout << "before reg a " << bananaCPU.registers[reg_a] << " reg b
        // " << bananaCPU.registers[reg_b] << "reg c " <<
        // bananaCPU.registers[reg_c] << std::endl;
        uint32_t shift_value = instruction << 21;
        shift_value = shift_value >> 27;

        (bananaCPU.ROptable[function])(bananaCPU, reg_a, reg_b, reg_c,
                                       shift_value);
        // std::cout << "after reg a " << bananaCPU.registers[reg_a] << " reg b
        // " << bananaCPU.registers[reg_b] << "reg c " <<
        // bananaCPU.registers[reg_c] << std::endl;
      }
    } else {
      // std::cout << "before reg a " << bananaCPU.registers[reg_a] << " reg b "
      // << bananaCPU.registers[reg_b] << std::endl;
